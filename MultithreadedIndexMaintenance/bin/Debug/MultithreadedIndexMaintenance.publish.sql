...
CREATE TABLE [dbo].[Runners] (
    [RunnerID]   SMALLINT         NOT NULL,
    [JobName]    UNIQUEIDENTIFIER NOT NULL,
    [JobID]      INT              NOT NULL,
...
BEGIN
	...
	Select  @GUID = NEWID()
		...
		, @WatcherJobName = @JobName + '_Watcher'
	...
	Select @RunnerID = RunnerID
		, @DefragCmd = N'Exec dbo.usp_DefragIndexes @RunnerID = ' + RunnerID
		From dbo.Runners
	...
	--Exec dbo.usp_CreateWatcherForRunner @RunnerID = @RunnerID, @WatchJobName = @JobName
...
'
	...
	Select @RunnerCount = Count(RunnerID) 
		from dbo.Runners
	...
	set
		...
		i.RunnerID = (b.RowNum % @RunnerCount),
		i.StatusID = 0
	From
		dbo.FragmentedIndexes as i
		inner join (select *, 
			ROW_NUMBER() OVER(order by IndexID asc, PageCount desc) as RowNum
			from dbo.FragmentedIndexes ) as b
		on b.Id = i.Id
